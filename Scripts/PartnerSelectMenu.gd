...
export (Array, Resource) var partner_data
...
func parse_ranger_data():
	...
	if partner_data.size() > 0:		
		for data in partner_data:
			recruits.push_back(data.get_snapshot())	
	if recruits.size() > 0:		
		recruits.invert()
...
func add_trainee_buttons(recruits:Array):
	...
	for recruit in recruits:
		...
		sprite.colors = recruit.human_colors if typeof(recruit.human_colors) == TYPE_DICTIONARY else JSON.parse(recruit.human_colors).result
		sprite.part_names = recruit.human_part_names if typeof(recruit.human_part_names) == TYPE_DICTIONARY else JSON.parse(recruit.human_part_names).result				
...
func load_fighter(result):
	...
	world_human_sprite.colors = result.human_colors if typeof(result.human_colors) == TYPE_DICTIONARY else JSON.parse(result.human_colors).result
	world_human_sprite.part_names = result.human_part_names if typeof(result.human_part_names) == TYPE_DICTIONARY else JSON.parse(result.human_part_names).result
	battle_human_sprite.part_names = result.human_part_names if typeof(result.human_part_names) == TYPE_DICTIONARY else JSON.parse(result.human_part_names).result
	battle_human_sprite.colors = result.human_colors if typeof(result.human_colors) == TYPE_DICTIONARY else JSON.parse(result.human_colors).result
...
func _on_ExitButton_pressed():
	if yield (MenuHelper.confirm("Are you sure you want to quit?"),"completed"):
		choose_option(null)
...
func _on_ActionButtons_option_chosen(option):
	...
	if option == "select_partner":
		co = _on_select_trainee_option_chosen(current_recruit)
...
func _on_select_trainee_option_chosen(chosen_recruit):
	yield (GlobalMessageDialog.show_message(Loc.tr(chosen_recruit.name) + " was added to the Party!"),"completed")
	choose_option(chosen_recruit)
